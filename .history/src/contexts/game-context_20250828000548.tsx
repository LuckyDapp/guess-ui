...
export const GameContextProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const startAggressivePolling = useCallback(() => {
        ...
        const pollWithBackoff = () => {
            ...
            refreshGameState().then((currentGame) => {
                ...
                // Check if we still have pending guesses
                const currentAttempts = attempts.filter(a => a.gameNumber === currentGame.game_number);
                const hasPendingGuesses = currentAttempts.some(attempt => attempt.clue == undefined);

                if (!hasPendingGuesses) {
        ...
        pollWithBackoff();
    }, [isPollingAggressively, refreshGameState, attempts]);

    // Function to check if we need to start polling
    ...
    }, [game, attempts, isPollingAggressively, startAggressivePolling]);
    ...
    useEffect(() => {
        if (!isPollingAggressively) {
            pollingIntervalRef.current = setInterval(() => {
                refreshGameState().then(() => {
                    // Check for pending guesses after regular refresh
                    setTimeout(() => checkPendingGuesses(), 100);
                });
            }, 30 * 1000);
        }
