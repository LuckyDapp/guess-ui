...
export function ContractExplorer() {
  ...
  const searchContractTransactions = async (): Promise<Transaction[]> => {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(blocksToSearch, currentBlockNumber); i++) {
        const blockNumber = currentBlockNumber - i;

        // Update progress
        const elapsed = Date.now() - startTime;
        const progress = (i / Math.min(blocksToSearch, currentBlockNumber)) * 100;
        const eta = i > 0 ? (elapsed / i) * (Math.min(blocksToSearch, currentBlockNumber) - i) : 0;

        setSearchProgress({
          current: i + 1,
          total: Math.min(blocksToSearch, currentBlockNumber),
          eta: Math.ceil(eta / 1000) // Convert to seconds
        });
        ...
        try {
          ...
          if (block && block.block && block.block.extrinsics) {
            ...
            for (let extrinsicIndex = 0; extrinsicIndex < block.block.extrinsics.length; extrinsicIndex++) {
              ...
              if (extrinsic.call && extrinsic.call.callName === 'call') {
                ...
                if (callArgs && callArgs.dest && callArgs.dest.toLowerCase() === contractAddress.toLowerCase()) {
                  ...
                  // Get extrinsic details for more info
      ...
      console.log('✅ Connected to Pop Network! No real transactions found in recent blocks, using demo data.');
      console.log(`Searched through ${Math.min(blocksToSearch, currentBlockNumber)} blocks`);
      return [];
    } catch (error) {
            ...
            <Box sx={{ mb: 3, p: 2, border: '1px solid rgba(100, 181, 246, 0.3)', borderRadius: 2 }}>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, mb: 2 }}>
                <TextField
                  ...
                  sx={{ minWidth: 150 }}
                  helperText={`~${Math.ceil((searchParams.blocksToSearch * 6) / 60)} min search time`}
                />
              ...
              {/* Search Info */}
              <Box sx={{ mt: 2, p: 1, backgroundColor: 'rgba(100, 181, 246, 0.1)', borderRadius: 1 }}>
                <Typography variant="body2" sx={{ color: '#64b5f6' }}>
                  📊 Will search through ~{searchParams.blocksToSearch.toLocaleString()} blocks
                  {currentBlock > 0 && ` (from block ${Math.max(0, currentBlock - searchParams.blocksToSearch)} to ${currentBlock})`}
                  <br />
                  ⏱️ Estimated search time: ~{Math.ceil((searchParams.blocksToSearch * 6) / 60)} minutes
                  {searchParams.blocksToSearch > 10000 && (
                    <span style={{ color: '#ff9800' }}>
                      <br />⚠️ Large search may take several minutes and consume network resources
                    </span>
                  )}
                </Typography>
              </Box>
