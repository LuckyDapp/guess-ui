...
export function ContractExplorer() {
  ...
  const searchContractTransactions = async (): Promise<Transaction[]> => {
    ...
    try {
      ...
      throw new Error('Pop Network uses Substrate API. Direct RPC calls for transaction history require specialized Substrate queries. Using demo data for now.');
    } catch (error) {
              ...
              <Box sx={{ mb: 3, p: 2, backgroundColor: 'rgba(0, 0, 0, 0.2)', borderRadius: '8px' }}>
                ...
                {!isUsingRealData && (
                  <Typography variant="caption" sx={{ color: '#ff9800', mt: 1, display: 'block' }}>
                    💡 Using demo data. Connect to Pop Network to see real transactions.
                  </Typography>
                )}
              </Box>
