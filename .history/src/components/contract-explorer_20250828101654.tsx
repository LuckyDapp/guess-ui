...
import { useContract, useChainId } from '@reactive-dot/react';
...
export function ContractExplorer() {
  ...
  const contract = useContract(gtnContract, contractAddress);
  ...
  const loadContractState = async () => {
    ...
    setLoading(true);
    try {
      const state = await contract.getCurrentGame(context?.game ? 'mock_signer' : undefined);
  ...
  const callContractFunction = async (functionName: string, params: any[] = []) => {
    ...
    try {
      ...
      switch (functionName) {
        case 'getCurrentGame':
          result = await contract.getCurrentGame();
          break;
        default:
      ...
      setCallResult({ function: functionName, params, error: error.message });
    } finally {
