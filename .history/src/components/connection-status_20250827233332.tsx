...
export function ConnectionStatus({ compact = false }: ConnectionStatusProps) {
  ...
  const getConnectionStatus = () => {
    ...
    const networkName = chainId === 'pop' ? 'Pop Network' : chainId.toUpperCase();
    return {
  ...
  return (
    ...
    >
      ...
      <Typography variant="body2" sx={{ color: '#b0b0b0', mb: 2 }}>
        {chainId && signer ? (
          <>
            Network: <span style={{ color: '#64b5f6' }}>{chainId === 'pop' ? 'Pop Network (Polkadot)' : chainId.toUpperCase()}</span><br/>
            Account: <span style={{ color: '#64b5f6' }}>{signer.address?.slice(0, 6)}...{signer.address?.slice(-4)}</span>
          </>
        ) : (
