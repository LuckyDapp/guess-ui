  ...
  const searchContractTransactions = async (): Promise<Transaction[]> => {
    ...
    try {
      ...
      async function processBlock(blockNumber: number, index: number): Promise<void> {
        try {
          // TEMPORARY: Add mock transactions for testing
          if (Math.random() < 0.3) { // 30% chance to add a mock transaction
            ...
            console.log(`🎯 Added mock transaction for block ${blockNumber}`);
          }
          ...
          console.log(`📦 Block ${blockNumber} raw response:`, JSON.stringify(block, null, 2));
          ...
          if (block && block.block && block.block.extrinsics) {
            ...
            for (let extrinsicIndex = 0; extrinsicIndex < block.block.extrinsics.length; extrinsicIndex++) {
              ...
              // Debug log for each extrinsic - log the full structure first
              console.log(`📋 Extrinsic ${extrinsicIndex} full structure:`, JSON.stringify(extrinsic, null, 2));
